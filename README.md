# Посмотреть можно [здесь](https://students-list.vladislavsurin.ru/) (для полноценной работы требуется запуск сервера. Инструкция ниже.)

# REST API для базы данных студентов

Перед запуском убедитесь, что вы установили Node.js версии 12 или выше.

Для запуска сервера перейдите в папку `backend` в репозитории и выполните команду `node index`. Для остановки нажмите сочетание клавиш CTRL+C. Важно разделить запуск сервера и клиентской части. Запустите папку сервера, как отдельный проект и отельно клиентскую часть.

После запуска сервера API будет доступен по адрусу `http://localhost:3300`.
